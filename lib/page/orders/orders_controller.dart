...
class OrdersController extends GetxController {
  ...
  Future<void> obtenerPedidosPendientes() async {
    try {
      Uri uri = Uri.parse(
        '$defaultApiServer/ordenes/obtenerListaPedidosPendientes/',
      );
      ...
      if (response.statusCode == 200) {
        ...
        if (data['success'] == true && data.containsKey('pedidosPorMesa')) {
          ...
          pedidosPendientes.value = List<Map<String, dynamic>>.from(
            data['pedidosPorMesa'],
          );
          ...
          for (var i = 0; i < pedidosPendientes.length; i++) {
            ...
            if (mesaPedido['items'] != null) {
              ...
              for (var j = 0; j < items.length; j++) {
                final item = items[j];
                print(
                  '    ${j + 1}. ${item['nombre']} x${item['cantidad']} - Estado: ${item['estado']}',
                );
              }
            }
          }
          ...
          print(
            '  Items individuales en carousel: ${pedidosIndividuales.length}\n',
          );
        }
        // ✅ CASO 3: Estructura alternativa sin campo "success"
        else if (data.containsKey('pedidosPorMesa')) {
          print(
            'ℹ️ Respuesta sin campo "success", procesando de todas formas...\n',
          );
        ...
        if (_autoRefreshTimer?.isActive != true) {
          QuickAlert.show(
            ...
            title: 'Error',
            text:
                'No se pudieron cargar los pedidos pendientes (${response.statusCode})',
            confirmBtnText: 'OK',
  ...
  void _extraerPedidosIndividuales() {
    ...
    detallesFlat.sort((a, b) {
      try {
        ...
        return fechaA.compareTo(
          fechaB,
        ); // ✅ CAMBIADO: fechaA con fechaB (era fechaB con fechaA)
      } catch (e) {
        return 0;
      }
    });
    ...
    if (detallesFlat.isNotEmpty) {
      print(
        '  Primer item: ${detallesFlat.first['nombreProducto']} (Mesa ${detallesFlat.first['numeroMesa']})',
      );
      print('  Fecha más reciente: ${detallesFlat.first['fecha']}');

      // Mostrar resumen de mesas
      Set<int> mesasUnicas = detallesFlat
          .map((item) => item['numeroMesa'] as int)
          .toSet();
  ...
  void mostrarModalEstadoOrden(int detalleId) {
    print(detalleId);
    Get.dialog(OrderStatusModal(pedidoId: detalleId));
  }
  ...
  Future<void> obtenerMesasConPedidosAbiertos() async {
    try {
        ...
        if (data.containsKey('success') && data['success'] == true) {
          ...
          for (var mesa in mesas) {
            ...
            for (var pedido in pedidosMesa) {
              ...
              double totalPedido = detallesFormateados.fold(
                0.0,
                (sum, d) =>
                    sum +
                    ((d['precioUnitario'] as num) * (d['cantidad'] as int)),
              );
            ...
            final nombreOrdenMesa = pedidosFormateados.isNotEmpty
                ? (pedidosFormateados.first['nombreOrden'] ??
                      'Mesa $numeroMesa')
                : 'Mesa $numeroMesa';
      ...
      if (!e.toString().contains('No hay mesas') &&
    ...
    for (var pedido in pedidos) {
      ...
      for (var detalle in detalles) {
        ...
        if ((detalle['statusDetalle'] ?? 'proceso') != 'cancelado') {
          total +=
              (detalle['precioUnitario'] ?? 0.0) * (detalle['cantidad'] ?? 1);
        }
      }
    }
  ...
  Future<Map<String, dynamic>?> procesarDetalleId(int detalleId) async {
    try {
      Uri uri = Uri.parse(
        '$defaultApiServer/ordenes/CompletarYTotalPedido/$detalleId/',
      );
      ...
      print(
        '📡 Respuesta para detalleId $detalleId - Código: ${response.statusCode}',
      );
    ...
    try {
      ...
      if (fallidos == 0) {
        ...
        QuickAlert.show(
          ...
          title: 'Pago Exitoso',
          text:
              'Mesa $numeroMesa pagada correctamente\n'
        ...
        QuickAlert.show(
          ...
          title: 'Pago Parcial',
          text:
              'Mesa $numeroMesa procesada parcialmente\n'
  ...
  void confirmarPago(int numeroMesa, double total) {
    ...
    QuickAlert.show(
      ...
      text:
          '¿Confirmar el pago de la Mesa $numeroMesa?\n\n'
  ...
  Future<void> atenderTodosLosPedidosMesa(int numeroMesa) async {
    try {
      ...
      final pedidosPendientes = pedidos.where((pedido) {
        final detalles = pedido['detalles'] as List;
        return detalles.any(
          (detalle) => (detalle['statusDetalle'] ?? 'proceso') == 'proceso',
        );
      }).toList();
  ...
  Future<void> liberarTodasLasMesas() async {
    ...
    try {
      ...
      final totalItems = mesasConPendientes.fold<int>(
        0,
        (sum, mesa) => sum + (mesa['itemsPendientes'] as int),
      );

      // Mostrar diálogo de confirmación
      String detallesMesas = mesasConPendientes
          .map(
            (mesa) =>
  ...
  dynamic _extraerIdMesa(Map<String, dynamic> mesa, int numeroMesa) {
    ...
    print(
      '⚠️ No se encontró ID específico para mesa $numeroMesa, usando numeroMesa como fallback',
    );
    return numeroMesa;
  }

  Future<void> _ejecutarLiberacionMasiva(
    List<Map<String, dynamic>> mesasConPendientes,
  ) async {
    try {
      ...
      for (var mesaInfo in mesasConPendientes) {
        ...
        try {
          ...
          Uri uri = Uri.parse(
            '$defaultApiServer/mesas/$numeroMesa/atender-todo/',
          );
      ...
      // Mostrar resultados
      if (fallidas == 0) {
        final totalItemsProcesados = resultados.fold<int>(
          0,
          (sum, resultado) => sum + (resultado['itemsProcesados'] as int? ?? 0),
        );
      } else if (exitosas > 0) {
        final mesasExitosas = resultados
            .where((r) => r['success'] == true)
            .map((r) => 'Mesa ${r['numeroMesa']}')
            .join(', ');

        final erroresDetallados = resultados
            .where((r) => r['success'] == false)
            .map((r) => '• Mesa ${r['numeroMesa']}: ${r['error']}')
            .join('\n');
  ...
  Future<void> liberarMesasCompletadas() async {
    ...
    try {
      ...
      for (var mesa in mesasConPedidos) {
        ...
        if (sinProceso && tieneCompletados) {
          mesasCompletadas.add({'numeroMesa': numeroMesa, 'mesa': mesa});
      ...
      QuickAlert.show(
        ...
        text:
            '¿Está seguro de que quiere liberar todas las mesas con pedidos completados?\n\n'
  ...
  Future<void> _ejecutarLiberacionMesasCompletadas(
    List<Map<String, dynamic>> mesasCompletadas,
  ) async {
    try {
      isLiberandoTodasLasMesas.value = true;
      ...
      for (var mesaInfo in mesasCompletadas) {
        ...
        try {
          ...
          if (response.statusCode == 200) {
            ...
            if (data['success'] == true) {
              exitosas++;
              print('✅ ${esGrupo ? "Grupo" : "Mesa"} $numeroMesa liberada');
            } else {
              fallidas++;
              mesasFallidas.add('${esGrupo ? "Grupo" : "Mesa"} $numeroMesa');
              print('❌ Error: ${data['message'] ?? 'Error desconocido'}');
            }
          } else {
            fallidas++;
            mesasFallidas.add('${esGrupo ? "Grupo" : "Mesa"} $numeroMesa');
      ...
      if (fallidas == 0) {
        ...
        QuickAlert.show(
          ...
          type: QuickAlertType.error,
          title: 'Error en Liberación',
          text: '❌ No se pudo liberar ninguna.\nVerifica la conexión.',
          confirmBtnText: 'Entendido',
          confirmBtnColor: Color(0xFFE74C3C),
        );
      }
