...
class _TableDetailsModalState extends State<TableDetailsModal> {
  ...
  // ✅ NUEVO: Variable para controlar actualizaciones
  bool _isUpdating = false;

  @override
  void initState() {
    super.initState();
    // ✅ NUEVO: Escuchar cambios en el OrdersController
 ...
 void _setupOrdersListener() {
    try {
      ...
      // Escuchar cuando se actualicen las órdenes
  ...
  Widget build(BuildContext context) {
    // ✅ NUEVO: Obtener datos actualizados dinámicamente
    final ordersController = Get.find<OrdersController>();
    
    // Buscar la mesa actualizada en los datos más recientes
    final mesaActualizada = ordersController.mesasConPedidos.firstWhere(
      (mesa) => mesa['numeroMesa'] == widget.mesa['numeroMesa'],
      orElse: () => widget.mesa, // Fallback a datos originales
    );
    ...
    return Column(
      children: [
        ...
        // Header del modal (tu código existente)
        Container(
        ...
        // Resto de tu interfaz usando los datos actualizados
        if (pedidos.length > 1)
                ...
                children: [
                  ...
                  Expanded(
                    child: ElevatedButton.icon(
                      ...
                      icon: Icon(Icons.add_shopping_cart, color: Colors.white, size: 16),
                      label: Text(
                        'AGREGAR PRODUCTOS',
                        style: TextStyle(
                          fontSize: 12,
                  ...
                  // Botón de pago (tu código existente)
...
// ✅ PASO 2: También agregar botón en _buildOrderCard para la vista de todos los pedidos
  ...
  return Card(
    ...
    child: Container(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          // Header del pedido con botón expandir (tu código existente)
          InkWell(
            onTap: () {
              setState(() {
                selectedOrderIndex = index;
          ...
          if (detalles.isNotEmpty) ...[
            SizedBox(height: 12),
            // Resumen de items (máximo 3) - tu código existente
            ...
            // ✅ MODIFICADO: Fila con total y botones de acción
            Column(
              children: [
                // Total
                Row(
                  ...
                  children: [
                    ...
                    if (total > 0 && _puedeSerPagado(pedido))
                      Expanded(
                        child: ElevatedButton.icon(
                          onPressed: () => _confirmarPagoPedido(pedido, total),
                          icon: Icon(Icons.payment, color: Colors.white, size: 14),
                          label: Text(
                            'PAGAR',
                            style: TextStyle(
                              fontSize: 11,
            ...
            SizedBox(height: 8),
            // Botón agregar productos para pedidos vacíos
            SizedBox(
...
void _abrirModalAgregarProductos(Map<String, dynamic> pedido) {
  ...
  // Imports necesarios (agregar al top del archivo):
  // import 'package:restaurapp/page/orders/add_products/add_products_to_order_controller.dart';
  // import 'package:restaurapp/page/orders/add_products/add_products_to_order_screen.dart';
  ...
  showModalBottomSheet(
    ...
    // Cuando se cierre el modal, refrescar los datos
    final controller = Get.find<OrdersController>();
    controller.refrescarDatos();
  });
  
  // Inicializar el controller con los datos del pedido después de mostrar el modal
  Future.delayed(Duration(milliseconds: 100), () {
    try {
      ...
      // Si el controller no existe, crearlo
